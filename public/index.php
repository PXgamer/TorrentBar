...
function trck_BF_DrawText($im, $x, $y, $font, $text)
{
    ...
    for ($i = 0; $i < strlen($text); $i++) {
        ...
        imagecopy($im, $font["font"], $xc, $yc, $sx, 0, trck_BF_LetterWidth($font, $text[$i]),
            trck_BF_TextHeight($font));
...
class TorrentBar
{
    ...
    public function createBaseLayer($path_to_baselayerImage)
    {
        ...
        $this->img = @imagecreatefrompng($path_to_baselayerImage) or $this->throwException(__FUNCTION__, 2,
            'Can\'t create base layer image');
    ...
    {
        $layer = @imagecreatefrompng($path_to_layerImage) or $this->throwException(__FUNCTION__, 3,
            'Can\'t create layer image');
        @imagecopy($this->img, $layer, $x, $y, 0, 0, imagesx($layer),
            imagesy($layer)) or $this->throwException(__FUNCTION__, 4, 'Can\'t copy layer image to base layer image');
    ...
    private function mysql_init()
    {
        ...
        if ($dbpasswd != '') {
            $link = @mysql_connect($dbhost, $dbuser, $dbpasswd) or $this->throwException(__FUNCTION__, 9,
                'Can\'t connect to database');
        } else {
            $link = @mysql_connect($dbhost, $dbuser) or $this->throwException(__FUNCTION__, 10,
                'Can\'t connect to database');
        }
    ...
    public function drawText($x, $y, $text)
    {
        ...
        @imagecopy($this->img, $img, $x, $y, 0, 0, imagesx($img), imagesy($img)) or $this->throwException(__FUNCTION__,
            12, 'Can\'t copy temporary image to base layer image');
    }
